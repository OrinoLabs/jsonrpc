...
type ErrorHandler = (req: Request, res: Response, err: JsonRpcError) => void;
...
export function createHandler(
  ...
  return function(httpRequest: Request, httpResponse: Response) {
    ...
    function sendResponse(jsonrpcResponse: object) {
      httpResponse.setHeader('Content-Type', 'application/json; charset=utf-8');
      httpResponse.write(JSON.stringify(jsonrpcResponse));
      httpResponse.end();
    }

    function replyWithResult(result: object) {
      sendResponse({'id': callId, 'result': result});
    }

    function replyWithError(error: JsonRpcError) {
